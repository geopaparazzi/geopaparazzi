...
public class OGCRequestDescription extends AbstractXMLEventParser {
	...
	public XMLEventParser allocate(XMLEventParserContext ctx, XMLEvent event) {
		...
		if (ctx.isStartElement(event, DCPTYPE)) defaultParser = new OGCDCType(this.getNamespaceURI());

		return ctx.allocate(event, defaultParser);
	}

	public Object parse(XMLEventParserContext ctx, XMLEvent rqstEvent, Object... args) throws XMLParserException {
		if (this.formats != null) this.formats.clear();
		if (this.dcpTypes != null) this.dcpTypes.clear();

		if (rqstEvent.isStartElement()) this.setRequestName(rqstEvent.getName().getLocalPart());
	...
	protected void doParseEventContent(XMLEventParserContext ctx, XMLEvent event, Object... args) throws XMLParserException {
		if (ctx.isStartElement(event, FORMAT)) {
			...
			if (!WWUtil.isEmpty(s)) this.addFormat(s);
		} else if (ctx.isStartElement(event, DCPTYPE)) {
			XMLEventParser parser = this.allocate(ctx, event);
			if (parser != null) {
				Object o = parser.parse(ctx, event, args);
				if (o != null && o instanceof OGCDCType) this.addDCPType((OGCDCType) o);
			}
		}
	}
	...
	public Set<String> getFormats() {
		if (this.formats != null) return formats;
	...
	protected void addFormat(String format) {
		if (this.formats == null) this.formats = new HashSet<String>();
	...
	public Set<OGCDCType> getDCPTypes() {
		if (this.dcpTypes != null) return dcpTypes;
		else return Collections.emptySet();
	}

	public void addDCPType(OGCDCType dct) {
		if (this.dcpTypes == null) this.dcpTypes = new HashSet<OGCDCType>();
	...
	public String toString() {
		...
		if (this.getRequestName() != null) sb.append(this.getRequestName()).append("\n");
