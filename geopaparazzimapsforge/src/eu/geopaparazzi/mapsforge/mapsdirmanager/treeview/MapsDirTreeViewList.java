...
public class MapsDirTreeViewList extends Activity {
    ...
    public static List<NodeObjectInfo> setMapTypeClasses( final List<NodeObjectInfo> maptype_classes_parm, final File maps_dir_parm ) {
        ...
        for( int i = 0; i < maptype_classes_parm.size(); i++ ) {
            ...
            // '/mnt/sdcard/maps/opencycle.mapurl' = level[1] or
            // '/mnt/extSdCard/maps/defaulttiles/_opencycle.mbtiles' = level[2]
            ...
            // GPLog.androidLog(-1,TAG+" setMapTypeClasses["+this_classinfo.getId()+"] [" +
            // this_classinfo.getFileNamePath()+ "]");
            // GPLog.androidLog(1,"MapsDirTreeViewList ["+i+"] i_level_filedirectory[" +
            // i_level_filedirectory+ "] map_file[" + s_map_file+"]");
        ...
        // GPLog.androidLog(2,"MapsDirTreeViewList setMapTypeClasses MAPDIR_LEVEL_NUMBER[" +
        // MAPDIR_LEVEL_NUMBER+ "] MAX_LEVEL_NUMBER[" + MAX_LEVEL_NUMBER+"]");
    ...
    public void onCreate( final Bundle savedInstanceState ) {
        ...
        if (savedInstanceState == null) {
                ...
                for( int i_class = 0; i_class < this.maptype_classes.size(); i_class++ ) {
                    ...
                    default: {
                        ...
                        if (this_classinfo.getLevel() > 1) { // level[1] is a file in the
                                                             // root-map-directory
                            s_directory_new = this_classinfo.getFilePath().getParent()
                                    .replaceAll(mapsdir_classinfo.getFileNamePath() + File.separator, "")
                                    + File.separator;
                        }
                        if (((this_classinfo.getLevel()) != i_level_maptype)
                                || (!s_directory_new.equals(s_directory_prev_maptype))) {
                            // GPLog.androidLog(-1,TAG+" onCreate[MAPTYPE] level["+this_classinfo.getLevel()+"] prev["
                            // + s_directory_prev_maptype+ "] new[" + s_directory_new+ "] ");
                            ...
                            if (!s_directory_new.equals("")) {
                                ...
                                for( int j = 0; j < sa_string.length; j++ ) { // if diff == 1:
                                                                              // '1861_World_Mercator/'
                                                                              // ; diff==2:
                                                                              // 'mbtiles/1861_World_Mercator'
                                    ...
                                    if (!s_directory.equals(s_directory_prev)) {
                                        ...
                                        l_position_maptype = (long) (i_class + i_parent_dir_maptype);
                                        tree_builder_maptype.sequentiallyAddNextNode(l_position_maptype, i_level_maptype,
                                                dir_classinfo);
                                        i_parent_dir_maptype++;
                                    }
                        ...
                        if (i_level_filedirectory == 0) {
                            ...
                            tree_builder_filedirectory.sequentiallyAddNextNode((long) i_parent_dir_filedirectory++,
                                    mapsdir_classinfo.getLevel(), mapsdir_classinfo);
                        ...
                        if (((this_classinfo.getLevel()) != i_level_filedirectory)
                            ...
                            if (!s_directory_new.equals("")) {
                                ...
                                for( int j = 0; j < sa_string.length; j++ ) { // if diff == 1:
                                    ...
                                    if (!s_directory.equals(s_directory_prev)) {
                                        ...
                                        tree_builder_filedirectory.sequentiallyAddNextNode(l_position_filedirectory,
                                                i_level_filedirectory, dir_classinfo);
                        ...
                        l_position_filedirectory = (long) (i_class + i_parent_dir_filedirectory);
                        tree_builder_filedirectory.sequentiallyAddNextNode(l_position_filedirectory, i_level_filedirectory,
                                this_classinfo);
                        // ---------------------------------------------
                        // End: 'DIRECTORY,FILE'
                        // ---------------------------------------------
            ...
            tree_manager_filedirectory = (TreeStateManager<Long>) savedInstanceState
        ...
        this_treeViewList.setOnItemClickListener(new OnItemClickListener(){
            public void onItemClick( AdapterView parent, View this_view, int position, long id_node ) {
                ...
                GPLog.androidLog(-1, TAG + " onItemClick[" + use_treeType + "] [" + node_info.getLongText() + "] id_node["
                        + id_node + "]");
    ...
    protected final void setTreeAdapter( final TreeType new_TreeType ) {
        ...
        default: {
            if (treeview_filedirectory == null) {
                treeview_filedirectory = new FileDirectoryTreeViewAdapter(this, selected_nodes, tree_manager_filedirectory,
                        MAX_LEVEL_NUMBER);
            }
