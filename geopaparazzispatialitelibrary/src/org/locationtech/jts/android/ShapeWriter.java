 ...
 */
package org.locationtech.jts.android;
...
import org.locationtech.jts.android.geom.PolygonShape;
...
public class ShapeWriter {
    ...
    public DrawableShape toShape(Geometry geometry) {
        if (geometry.isEmpty())
            return new PathShape(AndroidGraphicFactory.INSTANCE.createPath());
    ...
    private void appendRing(PolygonShape poly, Coordinate[] coords) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (!isFirst) {
                mainPath.lineTo(transPoint.x, transPoint.y);
            } else {
                mainPath.moveTo(transPoint.x, transPoint.y);
            }
        }
    ...
    private PathShape toShape(MultiLineString mls) {
        ...
        for (int i = 0; i < mls.getNumGeometries(); i++) {
            ...
            toShape(lineString, path);
        }
        return new PathShape(path);
    }

    private void toShape(LineString lineString, Path appendPath) {

        Coordinate prev = lineString.getCoordinateN(0);
        transformPoint(prev, transPoint);
        appendPath.moveTo((float) transPoint.x, (float) transPoint.y);
        ...
        for (int i = 1; i <= n; i++) {
            ...
            appendPath.lineTo((float) transPoint.x, (float) transPoint.y);
        }
    }
    ...
    private PathShape toShape(LineString lineString) {
        ...
        Coordinate prev = lineString.getCoordinateN(0);
        transformPoint(prev, transPoint);
        path.moveTo((float) transPoint.x, (float) transPoint.y);
        ...
        for (int i = 1; i <= n; i++) {
            ...
            path.lineTo((float) transPoint.x, (float) transPoint.y);
        }
        return new PathShape(path);
    }
