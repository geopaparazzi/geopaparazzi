...
public class MbTilesMetadata
{
 ...
 /**
   ...
   */
 public MbTilesMetadata(String name,String description,String type, String version,String format,float[] bounds,float[] center,
            int minZoom,int maxZoom,String s_tile_row_type,HashMap<String, String> extra)
 {
  this.name = name;
  this.type = type;
  ...
  if (center != null)
  {
   ...
   if ((center[2] < minZoom) || (center[2] > maxZoom))
    center[2]=minZoom; // .map files only have a minZoom, this will be used so that everything reacte in the same way
   this.center = center;
  }
  else
  {
   this.center=new float[]{this.bounds[0] + (this.bounds[2] - this.bounds[0]) / 2f,this.bounds[1] + (this.bounds[3] - this.bounds[1]) / 2f,maxZoom};
  }
  ...
  // SpatialDatabasesManager.app_log(-1,"MbTilesMetadata["+this.name+"] description["+this.description+"] bounds["+this.bounds[0]+","+this.bounds[1]+""+this.bounds[2]+""+this.bounds[3]+"] center_x_y["+this.center[0]+","+this.center[1]+"] zoom_default["+this.center[2]+"] tile_row_type["+this.s_tile_row_type+"]");
 }
 @Override
 ...
 /**
   ...
   * - mj10777: since this is a reader, we will attempt to supply default values for missing or incorrect values
   * - at the moment, only 1.1 should be called
   * - checking of mandatory fields should exist (to insure that this is really a mbtiles file)
 ...
 public static class MetadataValidator_1_1 implements MetadataValidator
 {
  ...
  public MbTilesMetadata validate(HashMap<String,String> hm) throws MetadataParseException
  {
   ...
   // bounds: optional
   // - should be set to zoom to this area when first loading the map, if the present point is out of range
   ...
   if (tmp != null)
   {
    ...
    if (center == null)
    {
     center=new float[]{bounds[0] + (bounds[2] - bounds[0]) / 2f,bounds[1] + (bounds[3] - bounds[1]) / 2f,maxZoom};
     // throw new MetadataParseException("Invalid syntax for optional field 'center'."
   ...
   { // Until this is accepted, other application will get 'confused' when the non-tms numbering is used
    s_tile_row_type = "tms";
   }
   return new MbTilesMetadata(name,description,type,version,format,bounds,center,minZoom,maxZoom,s_tile_row_type,hm);
  }
 }
 // -----------------------------------------------
 /**
 ...
 public static class MetadataValidator_1_0 implements MetadataValidator
 {
  ...
  public MbTilesMetadata validate(HashMap<String, String> hm) throws MetadataParseException
  {
   ...
   // -- user friendly, otherwise the user may be lost in a sea of white tiles.
    ...
    float[] bounds = {-180.0f,-85.05113f,180.0f,85.05113f};
    if (tmp != null)
    { // some tilemill db use this - despite the 1.0.0 version number
     bounds = ValidatorHelper.parseBounds(tmp);
     if (bounds == null)
     {
      bounds = new float[]{-180.0f,-85.05113f,180.0f,85.05113f};
 ...
 { // mj10777: i_rc=0=inside valid area/zoom ; i_rc > 0 outside area or zoom ; i_parm=0 no corrections ; 1= correct tileBounds values.
