...
public class DaoSpatialite {
    ...
    static {
       ...
       sb_query.append(" CASE WHEN data_type = 'features' THEN 'shape'"); // 1 of 1st field
       sb_query.append(" CASE WHEN data_type = 'tiles' THEN 'tile_data' "); // 1 of 1st field
       sb_query.append(" CASE WHEN data_type = 'featuresWithRasters' THEN 'location'"); // 1 of 1st field
       sb_query.append(" END ||';'||"); // 1 of 1st field
       sb_query.append(" CASE WHEN data_type = 'features' THEN 'GeoPackage_features'"); // 2 of 1st field
       sb_query.append(" CASE WHEN data_type = 'tiles' THEN 'GeoPackage_tiles'"); // 2 of 1st field
       sb_query.append(" CASE WHEN data_type = 'featuresWithRasters' THEN 'GeoPackage_featuresWithRasters'"); // 2 of 1st field
       sb_query.append(" END "); // 2 of 1st field
       sb_query.append("||';'||'ROWID'"); // 3 of 1st field
       sb_query.append(" CASE WHEN data_type = 'features' THEN '-1'"); // 4 of 1st field
       sb_query.append(" CASE WHEN data_type = 'tiles' THEN '-1"); // 4 of 1st field
       sb_query.append(" CASE WHEN data_type = 'featuresWithRasters' THEN 'picture'"); // 4 of 1st field
       sb_query.append(" END"); // 4 of 1st field
       sb_query.append(" AS vector_key,REPLACE(identifier,';','-')"); // 0 of second field
       sb_query.append("||';'||REPLACE(description,';','-')"); // 1 of second field
       sb_query.append(" CASE WHEN srid = '1' THEN '4326'"); // 2 of second field
       sb_query.append(" CASE WHEN srid = '2' THEN '3857"); // 2 of second field
    ...
    */
    ...
    public static String getJavaSqliteDescription( Database database, String name ) {
        ...
        try {
            ...
            s_javasqlite_description += "rasterlite2_properties[" + getRaster2Version(database) + "]]";
        } catch (Exception e) {
    ...
    public static String getRaster2Version( Database database ) throws Exception {
        Stmt stmt = database.prepare("SELECT RL2_Version()||';'||RL2_Target_CPU();");
        try {
            if (stmt.step()) {
                String value = stmt.column_string(0);
     ...
     * @throws Exception  if something goes wrong.
     */
    public static int SpatialiteRecoverSpatialIndex( Database database, String table_name, String geometry_column, int i_spatialindex,SpatialiteDatabaseType databaseType ) throws Exception {
     ...
     * @throws Exception  if something goes wrong.
     */
    public static int SpatialiteUpdateLayerStatistics( Database database, String table_name, String geometry_column, int i_spatialindex ,SpatialiteDatabaseType databaseType) throws Exception {
        if (i_spatialindex == 1)
        {
         try {
    ...
    private static void getSpatialVectorMap_V4( Database database, HashMap<String, String> spatialVectorMap , HashMap<String, String> spatialVectorMapErrors, boolean b_layers_statistics , boolean b_raster_coverages ) throws Exception {
        ...
        if ((VECTOR_LAYERS_QUERY_MODE > 0) && (spatialVectorMapErrors.size() > 0))
        { // if empty: there are nothing to correct [do befor RasterLite2 logic - there is no error control for that]
    ...
    public static SpatialiteDatabaseType checkDatabaseTypeAndValidity( Database database, HashMap<String, String> spatialVectorMap , HashMap<String, String> spatialVectorMapErrors )
        ...
        if (b_gpkg_contents) {
            ...
            if ((b_vector_layers_statistics) && (b_vector_layers)) { // Spatialite 4.0
             ...
             if ((b_geometry_columns)) { // Spatialite from 2.0 until 2.3
                    // avoids IllegalArgumentException
                    return SpatialiteDatabaseType.SPATIALITE2;
                }
              }
           }
        }
